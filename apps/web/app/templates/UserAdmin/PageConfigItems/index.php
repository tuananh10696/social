...
<?php $this->start('menu_list'); ?>
  ...
  <li class="breadcrumb-item active"><span>項目設定</span></li>
<?php $this->end(); ?>
...
<div class="table_list_area">
  <table class="table table-sm table-hover table-bordered dataTable dtr-inline" style="table-layout: fixed;">
    ...
    <tbody>
      ...
      <td>
        <?= $this->Html->link($data->parts_type, ['action' => 'edit', $data->id, '?' => $query], ['class' => 'btn btn-light w-100 text-left'])?>
      </td>

      <td>
      <?= $this->Html->link($data->item_key, ['action' => 'edit', $data->id, '?' => $query], ['class' => 'btn btn-light w-100 text-left'])?>
      </td>
